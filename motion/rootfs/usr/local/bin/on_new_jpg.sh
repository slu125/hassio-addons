...
if ($?VERBOSE && $?USE_MQTT) mosquitto_pub -u "${MOTION_MQTT_USERNAME}" -P "${MOTION_MQTT_PASSWORD}" -h "$MOTION_MQTT_HOST" -t "${MOTION_DEVICE_DB}/${MOTION_DEVICE_NAME}/debug" -m '{"INFO":"'$0:t'","pid":"'$$'","info":"moving '$image' to '$output'"}'
