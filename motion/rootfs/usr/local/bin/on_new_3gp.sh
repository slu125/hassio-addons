...
foreach j ( $jpgs )
  ...
  if ($?MOTION_FILE_NORMAL) then
    if ($?VERBOSE && $?USE_MQTT) mosquitto_pub -u "${MOTION_MQTT_USERNAME}" -P "${MOTION_MQTT_PASSWORD}" -h "$MOTION_MQTT_HOST" -t "${MOTION_DEVICE_DB}/${MOTION_DEVICE_NAME}/debug" -m '{"INFO":"'$0:t'","pid":"'$$'","info":"moving '$f' to '$output'"}'
...
if ($?json) then
  ...
  if ($?VERBOSE && $?USE_MQTT) mosquitto_pub -u "${MOTION_MQTT_USERNAME}" -P "${MOTION_MQTT_PASSWORD}" -h "$MOTION_MQTT_HOST" -t "${MOTION_DEVICE_DB}/${MOTION_DEVICE_NAME}/debug" -f "$json"
endif
